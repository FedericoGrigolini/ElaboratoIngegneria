...
import model.PrincipioAttivo;
import model.Regionale;
...
public class Tabella {
		...
		if(t instanceof Operatore){
			...
			try {
			      ...
			      String sql = "INSERT INTO Operatore (CodiceFiscale,Nome,Cognome,Data_N,Luogo_N,Data_Ass) " +
			                   ...
			                   "','"+ p.getLuogaNascita() + "','"+ p.getDataAssunzione() + "');"; 
			      stmt.executeUpdate(sql);
			      stmt.close();
			      c.commit();
			      c.close();
		...
		try {
		      ...
		      while ( rs.next() ) {
		    	  for(Operatore o : temp){
		  			if(o.getCodiceFiscale().equals(rs.getString("Operatore"))){
		  				Medico m = new Medico(o.getCodiceFiscale(), o.getNome(), o.getCognome(), o.getDataNascita(), o.getLuogaNascita(), o.getDataAssunzione(), rs.getString("Specialità"));
		  				result.add(m);
		  			}
		    	  }
		      }
		...
		if(t instanceof Medico){
			...
			try {
			      ...
			      String sql = "INSERT INTO Medico (Operatore,Specialità) " +
			                   "VALUES ('"+p.getCodiceFiscale() + "','"+p.getSpecialità()+ "');"; 
			      stmt.executeUpdate(sql);
			      stmt.close();
			      c.commit();
			      c.close();
	...
	public void insertInfermiere(Object t) {
		if(t instanceof Infermiere){
			...
			for(Operatore o:temp){
				if(o.getCodiceFiscale()==p.getCodiceFiscale()){
					try {
						Class.forName("org.sqlite.JDBC");
						c = DriverManager.getConnection("jdbc:sqlite:GestioneOspedale.db");
						c.setAutoCommit(false);
		...
		if(t instanceof Intervento){
			...
			try {
			      ...
			      String sql = "INSERT INTO Intervento (Ricovero,Codice_Intervento,Operatore,Data,Orario,Tipo,Livello_Urgenza,Durata,Anestesia) " +
			              "VALUES ('"+p.getRicovero().getCodiceUnivoco() + "','"+p.getCodiceIntervento() + "','" + p.getOperatore() + "','" 
